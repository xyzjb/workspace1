...
def get_nwp_list(column_list):
    return tuple([col.split('.')[0] for col in column_list
...
def add_feature_rho_crossed(x_df, col_name, concat=True):
    ...
    for nwp in nwp_list:
        if col_name == '.ws':
            tmp_dict[nwp + col_name + '_rho1'] = x_df[nwp + col_name] * x_df[nwp + '.rho']
            tmp_dict[nwp + col_name + '_rho2'] = x_df[nwp + col_name] ** 2 * x_df[nwp + '.rho']
            tmp_dict[nwp + col_name + '_angles'] = tmp_dict[nwp + col_name + '_rho1'] + tmp_dict[
                nwp + col_name + '_rho2']
        elif col_name == '.pre_ws':
            tmp_dict[nwp + col_name + '_rho1'] = x_df[nwp + col_name] * x_df[nwp + '.rho']
            tmp_dict[nwp + col_name + '_rho2'] = x_df[nwp + col_name] ** 2 * x_df[nwp + '.rho']
            tmp_dict[nwp + col_name + '_rho3'] = x_df[nwp + col_name] ** 3 * x_df[nwp + '.rho']
...
def polar_to_cart(x_df, concat=True):
    ...
    tmp_dict = {}
    for nwp in nwp_list:
        tmp_dict[nwp + '.u'] = x_df[nwp + '.ws'] * np.sin(x_df[nwp + '.wd'] / 180 * np.pi)
        tmp_dict[nwp + '.v'] = x_df[nwp + '.ws'] * np.cos(x_df[nwp + '.wd'] / 180 * np.pi)
    tmp_df = pd.DataFrame(tmp_dict)
    if concat:
        return pd.concat([x_df, tmp_df], axis=1)
    else:
